...
const UserCard: React.FC<UserCardProps> = () => {
  ...
  const getUserPosts = (postLogin: string) =>
    axios
      .get<Post[]>(`/api/getUserPosts?userForView=${postLogin}`, {
        auth: {
          username: login,
          password: '12345',
        },
  ...
  const renderPosts = () =>
    posts.map((post) => (
      <Grid key={post.id} xs={4} item>
        <PostCard afterLike={afterLike} {...post} />
      </Grid>
    ));
  ...
  return (
    <Grid container spacing={2}>
      <Grid item>
        <Typography variant="h5">Posts of {currentLogin}</Typography>
      </Grid>
      <Grid item container spacing={1}>
        ...
        <Grid item>
          <Button onClick={returnToMyPage}>Return to my page</Button>
        </Grid>
      </Grid>
      {login === currentLogin ? (
        <Grid item container spacing={1}>
